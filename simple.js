...
exports.makeWASocket = (connectionOptions, options = {}) => {
    let ptz = (global.opts['legacy'] ? makeWALegacySocket : makeWASocket)(connectionOptions)
    // ptz.ws.on('CB:stream:error', (stream) => {
    //     const { code } = stream || {}
    //     if (code == '401') ptz.ev.emit('connection.update', {
    ...
    function updateNameToDb(contacts) {
        ...
        for (const contact of contacts) {
            ...
            if (!chats) chats = ptz.chats[id] = { id }
            ptz.chats[id] = {
                ...chats,
                ...({
    ...
    ptz.ev.on('chats.set', async ({ chats }) => {
    ...
    ptz.ev.on('group-participants.update', async function updateParticipantsToDb({ id, participants, action }) {
        id = ptz.decodeJid(id)
        if (!(id in ptz.chats)) ptz.chats[id] = { id }
    ...
    ptz.ev.on('chats.upsert', async function chatsUpsertPushToDb(chatsUpsert) {
        ...
        if (!id) return
        let chats = ptz.chats[id] = { ...ptz.chats[id], ...chatsUpsert, isChats: true }
        const isGroup = id.endsWith('@g.us')
        if (isGroup) {
            const metadata = await ptz.groupMetadata(id).catch(_ => null)
            if (metadata) {
            ...
            for (const group in groups) ptz.chats[group] = { id: group, subject: groups[group].subject, isChats: true, metadata: groups[group] }
        }
    })
    ptz.ev.on('presence.update', async function presenceUpdatePushToDb({ id, presences }) {
        const sender = Object.keys(presences)[0] || id
        const _sender = ptz.decodeJid(sender)
        const presence = presences[sender]['lastKnownPresence'] || 'composing'
        let chats = ptz.chats[_sender]
        if (!chats) chats = ptz.chats[_sender] = { id: sender }
        chats.presences = presence
        if (id.endsWith('@g.us')) {
            let chats = ptz.chats[id]
            if (!chats) {
                const metadata = await ptz.groupMetadata(id).catch(_ => null)
                if (metadata) chats = ptz.chats[id] = { id, subject: metadata.subject, metadata }
            }
            chats.isChats = true
        }
    })
  ...
  /**
    ...
    */
    ptz.sendMedia = async (jid, path, quoted, options = {}) => {
        let { ext, mime, data } = await ptz.getFile(path)
        messageType = mime.split("/")[0]
        pase = messageType.replace('application', 'document') || messageType
        return await ptz.sendMessage(jid, { [`${pase}`]: data, mimetype: mime, ...options }, { quoted })
    }
    
    ptz.adReply = (jid, text, title = '', body = '', buffer, source = '', quoted, options) => {
                let { data } = ptz.getFile(buffer, true)
                return ptz.sendMessage(jid, { text: text, 
                    contextInfo: {
                        ...
                        externalAdReply: {
                            ...
                            title: title,
                            body: body,
                            thumbnailUrl: 'https://telegra.ph/file/6c17c3b22940e88548129.jpg',
    ...
    */
     ...
     * @param {Object} options 
     */
     ptz.sendContact = async (jid, data, quoted, options) => {
...
PHOTO;BASE64:${(await ptz.getFile(await ptz.profilePictureUrl(njid)).catch(_ => ({})) || {}).number?.toString('base64')}
...
`.trim()
    ...
    ptz.fakeReply = (jid, text = '', fakeJid = ptz.user.jid, fakeText = '', fakeGroupJid, options) => {
        return ptz.sendMessage(jid, { text: text }, { ephemeralExpiration: 86400, quoted: { key: { fromMe: fakeJid == ptz.user.jid, participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options } })
    }
    ...
    */
    ...
    ptz.copyNForward = async (jid, message, forwardingScore = true, options = {}) => {
        ...
        m = generateWAMessageFromContent(jid, m, { ...options, userJid: ptz.user.id })
        await ptz.relayMessage(jid, m.message, { messageId: m.key.id, additionalAttributes: { ...options } })
        return m
    }
    ...
    ptz.downloadM = async (m, type, saveToFile) => {
        ...
        if (saveToFile) var { filename } = await ptz.getFile(buffer, true)
    ...
    ptz.parseMention = (text = '') => {
     ...
     ptz.sendTextWithMentions = async (jid, text, quoted, options = {}) => ptz.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
    ...
    if (!chat || chat === 'status@broadcast') return
        const emitGroupUpdate = (update) => {
            ptz.ev.emit('groups.update', [{ id: chat, ...update }])
        }
        ...
        if (!chats) chats = ptz.chats[chat] = { id: chat }
        chats.isChats = true
    ...
    ptz.insertAllGroup = async() => {
        ...
        for (const group in groups) ptz.chats[group] = { ...(ptz.chats[group] || {}), id: group, subject: groups[group].subject, isChats: true, metadata: groups[group] }
            return ptz.chats
    }
    ...
    /*ptz.processMessageStubType = async (m) => {
        ...
        const chat = ptz.decodeJid(m.key.remoteJid || m.message[mtype] && m.message[mtype].groupId || '')
        const isGroup = chat.endsWith('@g.us')
        if (!isGroup) return
        let chats = ptz.chats[chat]
        if (!chats) chats = ptz.chats[chat] = { id: chat }
                ...
                if (!message) continue
                    ...
                    if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                        ...
                        if (isGroup && !participant) participant = remoteJid
                            ...
                            if (!qChats) qChats = ptz.chats[participant] = { id: participant, isChats: !isGroup }
                            ...
                            if (!chats) {
                                if (isGroup) await ptz.insertAllGroup().catch(console.error)
                                    chats = ptz.chats[chat] = { id: chat, isChats: true, ...(ptz.chats[chat] || {}) }
                            }
                            let metadata, sender
                            if (isGroup) {
                            ...
                            if (sender !== chat) {
                                ...
                                if (!chats) chats = ptz.chats[sender] = { id: sender }
...
ptz.sendPoll = async (jid, name = '', optiPoll, options) => {
    ...
    return ptz.relayMessage(jid, { pollCreationMessage: pollMessage }, { ...options });
};
    ...
    /*ptz.pushMessage = async (m) => {
        ...
        for (const message of m) {
            try {
                // if (!(message instanceof proto.WebMessageInfo)) continue // https://github.com/adiwajshing/Baileys/pull/696/commits/6a2cb5a4139d8eb0a75c4c4ea7ed52adc0aec20f
                if (!message) continue
                if (message.messageStubType) ptz.processMessageStubType(message).catch(console.error)
                let mtype = Object.keys(message.message || {})
                mtype = mtype[mtype[0] === 'messageContextInfo' && mtype.length == 2 ? 1 : 0]
                const chat = ptz.decodeJid(message.key.remoteJid || message.message[mtype] && message.message[mtype].groupId || '')
                const isGroup = chat.endsWith('@g.us')
                let chats = ptz.chats[chat]
                if (!chats) {
                    if (isGroup) {
                    ...
                    chats = ptz.chats[chat] = { id: chat, ...(ptz.chats[chat] || {}) }
                }
                let metadata, sender
                if (isGroup) {
                    if (!chats.subject || !chats.metadata) {
                    ...
                    sender = ptz.decodeJid(message.fromMe && ptz.user.id || message.participant || message.key.participant || chat || '')
                    if (sender !== chat) {
                        let chats = ptz.chats[sender]
                        if (!chats) chats = ptz.chats[sender] = { id: sender }
                        if (!chats.name) chats.name = message.pushName || chats.name || ''
                ...
                chats.isChats = true
                const fromMe = message.key.fromMe || areJidsSameUser(chat, ptz.user.id)
                if (!chats.messages) chats.messages = {}
 ...
 */
 exports.smsg = (ptz, m, hasParent) => {
    ...
    if (m.key) {
        ...
        m.chat = ptz.decodeJid(m.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
        m.isGroup = m.chat.endsWith('@g.us')
        m.sender = ptz.decodeJid(m.key.fromMe && ptz.user.id || m.participant || m.key.participant || m.chat || '')
        m.fromMe = m.key.fromMe || areJidsSameUser(m.sender, ptz.user.id)
    }
    if (m.message) {
        ...
        if (m.mtype == 'protocolMessage' && m.msg.key) {
            ...
            m.msg.key.fromMe = ptz.decodeJid(m.msg.key.participant) === ptz.decodeJid(ptz.user.id)
            if (!m.msg.key.fromMe && m.msg.key.remoteJid === ptz.decodeJid(ptz.user.id)) m.msg.key.remoteJid = m.sender
        }
        ...
        if (m.quoted) {
            ...
            m.quoted.chat = ptz.decodeJid(m.msg.contextInfo.remoteJid || m.chat || m.sender)
            m.quoted.isBaileys = m.quoted.id && m.quoted.id.length === 16 || false
            m.quoted.sender = ptz.decodeJid(m.msg.contextInfo.participant)
            m.quoted.fromMe = m.quoted.sender === ptz.user.jid
            m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.contentText || ''
            m.quoted.name = ptz.getName(m.quoted.sender)
            ...
            m.getQuotedObj = m.getQuotedMessage = async () => {
                ...
                let q = M.fromObject(await ptz.loadMessage(m.quoted.id) || vM)
                return exports.smsg(ptz, q)
            }
            if (m.quoted.url || m.quoted.directPath) m.quoted.download = (saveToFile = false) => ptz.downloadM(m.quoted, m.quoted.mtype.replace(/message/i, ''), saveToFile)
...
/*exports.smsg = (conn, m, hasParent) => {
    ...
    if (m.key) {
        ...
        m.chat = ptz.decodeJid(m.key.remoteJid || m.message[mtype] && m.message[mtype].groupId || '')
        m.isGroup = m.chat.endsWith('@g.us')
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.getQuotedObj = m.getQuotedMessage = () => {
                ...
                let q = M.fromObject(((ptz.chats[m.quoted.chat] || {}).messages || {})[m.quoted.id])
             ...
             * @param {Object} options
             */
            m.quoted.reply = (text, chatId, options) => ptz.reply(chatId ? chatId : m.chat, text, vM, options)

            /**
             * Copy quoted message
             */
            m.quoted.copy = () => exports.smsg(ptz, M.fromObject(M.toObject(vM)))
            ...
            */
            m.quoted.forward = (jid, forceForward = false) => ptz.forwardMessage(jid, vM, forceForward)
             ...
             * @param {Object} options
            */
            m.quoted.copyNForward = (jid, forceForward = true, options = {}) => ptz.copyNForward(jid, vM, forceForward, options)
             ...
             * @param {Object} options
            */
            m.quoted.cMod = (jid, text = '', sender = m.quoted.sender, options = {}) => ptz.cMod(jid, vM, text, sender, options)
    ...
    m.name = m.pushName || ptz.getName(m.sender)
    if (m.msg && m.msg.url) m.download = (saveToFile = false) => ptz.downloadM(m.msg, m.mtype.replace(/message/i, ''), saveToFile)
    /**
     ...
     * @param {Object} options
     */
    m.reply = (text, chatId, options) => ptz.reply(chatId ? chatId : m.chat, text, m, options)

    /**
     * Copy this message
     */
    m.copy = () => exports.smsg(ptz, M.fromObject(M.toObject(m)))
     ...
     * @param {Boolean} forceForward
     */
    m.forward = (jid = m.chat, forceForward = false) => ptz.copyNForward(jid, m, forceForward, options)
     ...
     * @param {Object} options
     */
     m.reply = async (text, chatId, options) => ptz.reply(chatId ? chatId : m.chat, text, m, options)
    ...
    return ptz.relayMessage(m.chat, msg.message, {
      contextInfo: {
    ...
    m.copyNForward = (jid = m.chat, forceForward = true, options = {}) => ptz.copyNForward(jid, m, forceForward, options)
     ...
     * @param {Object} options 
     */
    m.cMod = (jid, text = '', sender = m.sender, options = {}) => ptz.cMod(jid, m, text, sender, options)

    /**
     * Delete this message
     */
    m.delete = () => ptz.sendMessage(m.chat, { delete: m.key })

    try {
        if (m.msg && m.mtype == 'protocolMessage') ptz.ev.emit('message.delete', m.msg.key)
    } catch (e) {
        console.error(e)
    }
    return m
